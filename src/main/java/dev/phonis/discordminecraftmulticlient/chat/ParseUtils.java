...
class ParseUtils
{
	...
	public static
	String getRawMessage(String json)
	{
		try
		{
	...
	private static
	String translateString(String ruleName, List<String> usingData)
	{
		if (ParseUtils.translationRules.containsKey(ruleName))
		{
			...
			for (int i = 0; i < rule.length(); i++)
			{
				if (rule.charAt(i) == '%' && i + 1 < rule.length())
				{
					...
					else if (Character.isDigit(rule.charAt(i + 1)) && i + 3 < rule.length() &&
							 rule.charAt(i + 2) == '$' && (rule.charAt(i + 3) == 's' || rule.charAt(i + 3) == 'd'))
					{
	...
	private static
	String getRawString(JsonElement chatElement)
	{
	...
	private static
	void buildRawString(JsonElement chatElement, StringBuilder builder)
	{
